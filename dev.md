...
- **V3 binlog parser phase 1**: 只解析整数 + VARCHAR 列类型, 其他类型跳过为 TODO; UPDATE/DELETE 解析为 stub. 已支持自动剥离 4 字节 CRC32 校验和 (`binlog_checksum=NONE` 不再是必须). SHOW MASTER STATUS 在 MySQL 8.0.22+ 改为 SHOW BINARY LOG STATUS.
